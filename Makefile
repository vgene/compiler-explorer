...
run: prereqs webpack  ## Runs the site normally
	./node_modules/.bin/supervisor -w app.js,lib,etc/config -e 'js|node|properties|yaml' --exec $(NODE) $(NODE_ARGS) -- --experimental-modules --es-module-specifier-resolution=node ./app.js $(EXTRA_ARGS)
...
dev: prereqs install-git-hooks ## Runs the site as a developer; including live reload support and installation of git hooks
	./node_modules/.bin/supervisor -w app.js,lib,etc/config -e 'js|node|properties|yaml' -n exit --exec $(NODE) $(NODE_ARGS) -- --experimental-modules --es-module-specifier-resolution=node ./app.js $(EXTRA_ARGS)
...
debug: prereqs install-git-hooks ## Runs the site as a developer with full debugging; including live reload support and installation of git hooks
	./node_modules/.bin/supervisor -w app.js,lib,etc/config -e 'js|node|properties|yaml' -n exit --exec $(NODE) $(NODE_ARGS) -- --experimental-modules --es-module-specifier-resolution=node ./app.js --debug $(EXTRA_ARGS)
